...
function tabKey() {
	...
	// Also autocomplete theme names when typing "theme "
	if (currInput.startsWith("theme ")) {
		const themeInput = currInput.slice(6);
		for (const name of THEME_NAMES) {
			if (name.startsWith(themeInput)) {
				USERINPUT.value = "theme " + name;
				return;
			}
		}
		return;
...
function commandHandler(input: string) {
	...
	// Handle theme with argument
	if (input.startsWith("theme ")) {
		const themeName = input.slice(6).trim();
		if (applyTheme(themeName)) {
			writeLines(createThemeApplied(themeName));
		} else {
			writeLines([
				"<br>",
	...
	switch (input) {
		...
		case "banner":
			if (bareMode) {
				writeLines(["mikefisher.dev v1.0.0", "<br>"]);
				break;
			}
			writeLines(BANNER);
			break;
		...
		case "resume":
			if (bareMode) {
				writeLines(["the resume got deleted too.", "<br>"]);
				break;
			}
			writeLines(RESUME);
			break;
		case "experience":
			if (bareMode) {
				writeLines(["all that experience and you still ran rm -rf.", "<br>"]);
				break;
			}
		...
		case "neofetch":
			if (bareMode) {
				writeLines(["neofetch: command not found. (you deleted it)", "<br>"]);
				break;
			}
			writeLines(NEOFETCH);
			break;
		case "theme":
			writeLines(createThemeList());
			break;
		case "repo":
		...
		case "github":
			...
			break;
		case "email":
			writeLines(["Opening mail client...", "<br>"]);
			setTimeout(() => {
				window.open(`mailto:${command.social.email}`);
			}, 500);
			break;
		case "rm -rf":
			if (bareMode) {
